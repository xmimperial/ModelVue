...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="fixed top-6 left-6 w-80 z-40 space-y-4" aria-label="Properties and Settings">
      <Card className="bg-card/80 backdrop-blur-md border-border/50 shadow-2xl">
        ...
        <CardContent className="space-y-4">
          {isLoading ? (
            <div className="flex flex-col items-center py-8 space-y-3" role="status" aria-live="polite">
              <Loader2 className="w-8 h-8 animate-spin text-primary" />
              <p className="text-sm text-muted-foreground animate-pulse font-medium">Analyzing geometry...</p>
            </div>
          ) : error ? (
            <div className="flex flex-col items-center py-6 text-center space-y-4" role="alert">
            ...
            <Tabs defaultValue="info" className="w-full">
              <TabsList className="grid w-full grid-cols-2 mb-4 bg-muted/50" aria-label="Viewer Options">
      ...
      {!metadata && !isLoading && !error && (
        <Card className="bg-card/40 border-none shadow-none">
          <CardContent className="pt-6">
            <h3 className="text-[10px] uppercase font-bold text-muted-foreground mb-3 tracking-widest">Available Formats</h3>
            <ul className="flex flex-wrap gap-1.5" aria-label="Supported file formats">
              {['FBX', 'GLTF', 'GLB', 'OBJ', 'STL', 'PLY'].map(fmt => (
                <li key={fmt} className="px-2 py-0.5 rounded bg-secondary/50 text-[9px] font-code text-muted-foreground border border-border/30">
                  .{fmt.toLowerCase()}
                </li>
              ))}
              <li className="text-[9px] text-muted-foreground/60 italic self-center ml-1">+ more</li>
            </ul>
          </CardContent>
        </Card>
      )}
